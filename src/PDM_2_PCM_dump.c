...
//No need BCLK_TIMER output
	...
	// Configure the output pin.
	//
	am_hal_ctimer_output_config(BCLK_TIMER,
	                            BCLK_TIMER_SEG,
	                            BCLK_PIN,
	                            AM_HAL_CTIMER_OUTPUT_NORMAL,
...
main(void)
{
	...
	while (1)
	{
		...
		if(g_bPDMDataReady == true)
		{
			...
			//pdm_dump(i16PDMBuf[(u32PDMpg-1)%2],u32FrameSize);
			...
			if(ui32PinValue == 0)
			{
				if(ui32deBounce++ >= 3)
				{
					ret = storage_write(index, i16PDMBuf[(u32PDMpg-1)%2], u32FrameSize*2);
					if(ret)
						am_util_stdio_printf("storage_write failed \r\n");
					index += u32FrameSize*2; 
